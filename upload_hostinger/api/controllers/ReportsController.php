...
class ReportsController extends ApiController {
    ...
    public function getSales() {
        ...
        $stmt = $this->conn->prepare("
            SELECT s.*, c.name as customer_name,
                   (SELECT COUNT(*) FROM fiscal_notes fn WHERE fn.sale_id = s.id AND fn.status = 'generated') > 0 as has_fiscal_note,
                   (SELECT fn.status FROM fiscal_notes fn WHERE fn.sale_id = s.id ORDER BY fn.created_at DESC LIMIT 1) as fiscal_note_status,
                   (SELECT fn.id FROM fiscal_notes fn WHERE fn.sale_id = s.id AND fn.status = 'generated' ORDER BY fn.created_at DESC LIMIT 1) as fiscal_note_id
            FROM sales s 
    ...
    public function getTopProducts() {
        ...
        $stmt = $this->conn->prepare("
            SELECT p.name, c.name as category, SUM(si.quantity * COALESCE(si.multiplier, 1)) as total_qty, SUM(si.quantity * si.unit_price) as total_amount
            FROM sale_items si
    ...
    public function getStockReport() {
        ...
        $stmt = $this->conn->prepare("
            SELECT p.id, p.name, p.stock_current as stock, p.stock_min as min, c.name as category,
                   (SELECT IFNULL(SUM(si.quantity * COALESCE(si.multiplier, 1)), 0) FROM sale_items si WHERE si.product_id = p.id) as sold
            FROM products p
            LEFT JOIN categories c ON p.category_id = c.id
    ...
    public function getSalesByProduct() {
        ...
        $stmt = $this->conn->prepare("
            SELECT p.code, p.name, c.name as category, p.sub_group,
                   SUM(si.quantity * COALESCE(si.multiplier, 1)) as total_qty,
    ...
    public function getProfitability() {
        ...
        $stmt = $this->conn->prepare("
            SELECT p.id, p.code, p.name, c.name as category, p.sub_group,
                   SUM(si.quantity * COALESCE(si.multiplier, 1)) as total_qty,
                   SUM(si.quantity * si.unit_price) as total_revenue,
                   SUM((si.quantity * COALESCE(si.multiplier, 1)) * COALESCE(p.cost_price, 0)) as total_cost,
                   SUM((si.quantity * si.unit_price) - ((si.quantity * COALESCE(si.multiplier, 1)) * COALESCE(p.cost_price, 0))) as total_profit
            FROM sale_items si
            JOIN sales s ON si.sale_id = s.id
            JOIN products p ON si.product_id = p.id
            LEFT JOIN categories c ON p.category_id = c.id
    ...
    public function getPlanoContas() {
        ...
        // Receitas: contas do plano + valores pagos de accounts_receivable
        $stmt = $this->conn->prepare("
            SELECT
            ...
            LEFT JOIN accounts_receivable ar
                ON  ar.category    = coa.name
                AND ar.company_id  = :company_id
                AND ar.status      = 'paid'
                AND ar.payment_date BETWEEN :start AND :end
            WHERE coa.company_id = :company_id2
              AND coa.type       = 'revenue'
              AND coa.active     = 1
            GROUP BY coa.id, coa.name
            ORDER BY total DESC, coa.name
        ");
        $stmt->execute([
            ':company_id'  => $this->company_id,
            ':start'       => $start,
        ...
        // Vendas diretas (PDV/caixa) que NÃO possuem entrada em accounts_receivable
        $stmt = $this->conn->prepare("
            ...
            WHERE s.company_id = :company_id
              ...
              AND NOT EXISTS (
                  ...
                  WHERE ar.sale_id    = s.id
                    AND ar.company_id = s.company_id
              )
        ");
        $stmt->execute([':company_id' => $this->company_id, ':start' => $start, ':end' => $end]);
        $directSales = (float)($stmt->fetch()['total'] ?? 0);

        // Adiciona vendas diretas à conta de "Venda de Mercadorias" (ou cria linha avulsa)
        if ($directSales > 0) {
            $merged = false;
            foreach ($revenues as &$r) {
                if (stripos($r['account_name'], 'venda') !== false || stripos($r['account_name'], 'mercadoria') !== false) {
                    $r['total'] = (float)$r['total'] + $directSales;
                    $merged = true;
                    break;
                }
            }
            unset($r);
            if (!$merged) {
                $revenues[] = ['account_id' => null, 'account_name' => 'Vendas (PDV/Diretas)', 'total' => $directSales];
            }
        }

        // Despesas: contas do plano + valores pagos de accounts_payable
        $stmt = $this->conn->prepare("
            SELECT
                coa.id   AS account_id,
                coa.name AS account_name,
            ...
            LEFT JOIN accounts_payable ap
                ON  ap.category    = coa.name
                AND ap.company_id  = :company_id
                AND ap.status      = 'paid'
                AND ap.payment_date BETWEEN :start AND :end
            WHERE coa.company_id = :company_id2
        ...
        $totalRevenue = 0;
        foreach ($revenues as &$r) { $r['total'] = (float)$r['total']; $totalRevenue += $r['total']; }
        unset($r);

        $totalExpense = 0;
        foreach ($expenses as &$e) { $e['total'] = (float)$e['total']; $totalExpense += $e['total']; }
        unset($e);
