...
class FiscalService {
    ...
    public function generateNFe($sale, $items, $model = '55', $customerData = null, $ideData = null) {
        ...
        if (!empty($ideData['dhSaiEnt'])) {
            ...
            $ide->dSaiEnt = $dtSai->format('Y-m-d\TH:i:sP');
        } else {
            $ide->dSaiEnt = $ide->dhEmi; // padrão: igual à emissão
        }
        ...
        foreach ($items as $item) {
            ...
            // PIS (Simples Nacional - isento)
            $pis = new \stdClass();
            $pis->item = $n;
            $pis->CST = '07'; // 07 = Operação Isenta (correto para SN)
            $nfe->tagPIS($pis);

            // COFINS (Simples Nacional - isento)
            $cofins = new \stdClass();
            $cofins->item = $n;
            $cofins->CST = '07'; // 07 = Operação Isenta (correto para SN)
