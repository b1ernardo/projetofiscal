...
import { Plus, Trash2, CheckCircle, Clock, AlertCircle, Calendar, Search, X, Filter } from "lucide-react";
...
export default function ContasPagar() {
    ...
    const { data: accounts = [], isLoading } = useQuery<AccountPayable[]>({
        ...
        queryFn: async () => {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/accounts-payable`, {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            toast.success("Novo lançamento criado!");
            setFormOpen(false);
        },
    ...
    const payMutation = useMutation({
        mutationFn: async ({ account, paymentMethod }: { account: AccountPayable, paymentMethod: string }) => {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/accounts-payable/${account.id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
                    'Content-Type': 'application/json'
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: string) => {
            await fetch(`${import.meta.env.VITE_API_URL}/accounts-payable/${id}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${localStorage.getItem('auth_token')}` }
            });
        },
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'paid': return <Badge variant="secondary" className="bg-green-100 text-green-700 hover:bg-green-100"><CheckCircle className="mr-1 h-3 w-3" /> Pago</Badge>;
            case 'overdue': return <Badge variant="destructive"><AlertCircle className="mr-1 h-3 w-3" /> Atrasado</Badge>;
            default: return <Badge variant="outline" className="text-orange-600 border-orange-200 bg-orange-50"><Clock className="mr-1 h-3 w-3" /> Pendente</Badge>;
        }
    };
    ...
    const filteredAccounts = accounts.filter(acc => {
        ...
        const matchesSearch =
            ...
            (acc.supplier_name?.toLowerCase() || "").includes(searchQuery.toLowerCase());
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="bg-orange-50/50 border-orange-100">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-orange-600">Total Pendente</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                {/* Adicione mais cards de resumo se quiser */}
            </div>

            <Card className="border-none shadow-premium bg-card/80 backdrop-blur-sm">
                <CardHeader className="space-y-4">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <CardTitle className="text-lg text-orange-600">Listagem de Contas</CardTitle>
                        <div className="flex flex-wrap gap-2">
                            ...
                            <Button variant={filter === 'paid' ? 'default' : 'outline'} size="sm" onClick={() => setFilter('paid')}>Pagos</Button>
                        ...
                        <TableBody>
                            {filteredAccounts.map((acc) => (
                                <TableRow key={acc.id}>
                                    ...
                                    <TableCell>{acc.supplier_name || "—"}</TableCell>
                                    <TableCell><Badge variant="outline">{acc.category}</Badge></TableCell>
                                    <TableCell className="font-bold">{formatCurrency(acc.amount)}</TableCell>
                                    ...
                                    <TableCell className="text-right">
                                        <div className="flex justify-end gap-2">
                                            ...
                                            <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => {
                                                if (confirm("Deseja remover este lançamento?")) deleteMutation.mutate(acc.id)
                                            }}>
            ...
            {selectedAccount && (
                <PaymentConfirmationDialog
                    ...
                    onConfirm={(paymentMethod) => {
                        payMutation.mutate({ account: selectedAccount, paymentMethod });
                    }}
